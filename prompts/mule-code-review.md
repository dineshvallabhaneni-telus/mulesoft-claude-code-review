...
Review framework root:

$REVIEW_KIT_ROOT

The MuleSoft application source is located under:

$APPLICATION_ROOT

The review framework is located under:
...
IMPORTANT:
The review framework is NOT the MuleSoft application.

You MUST review the MuleSoft application under $APPLICATION_ROOT.

You MUST use the framework files under $REVIEW_KIT_ROOT.
...
This is the ONLY structured review output.
...
The final Word report will be generated by the framework after Claude completes.
...
5. Read applicable review references under:
   $REVIEW_KIT_ROOT/skills/mule-code-review/references/

6. Review the MuleSoft application under:
   $APPLICATION_ROOT
...
10. Validate that review.json is valid JSON before finishing.
...
After successfully creating review.json, respond briefly with:
