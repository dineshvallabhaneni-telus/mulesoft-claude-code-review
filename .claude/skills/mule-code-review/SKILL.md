...
The only file that may be created is ONE timestamped Word review report, `reports/CODE_REVIEW_REPORT_<YYYYMMDD-HHMMSS>.docx`, generated by piping the structured review JSON into `scripts/generate_report.py` (see `prompts/mule-full-review.md`, PHASE 19 and PHASE 20). The JSON schema is defined by `references/review-report-schema.md`, and the generator owns the document structure, the `reports/` location and the file name. Generate exactly one document per review run. Do not create `CODE_REVIEW_REPORT.md`, a findings JSON file, or any other intermediate report file.
...
In addition, `references/review-report-schema.md` defines the structured JSON
that the review is authored in and the Word document that is generated from
it. Read it before writing the review output.
